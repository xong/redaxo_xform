...
class rex_xform_value_month extends rex_xform_value_abstract
{
    ...
    $label = ($this->getElement(2) != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->getElement(2)) . '</label>' : '';	
		$field = '<input'.$classes.' id="'.$this->getFieldId().'" type="month" name="'.$this->getFieldName().'" value="'.htmlspecialchars(stripslashes($this->getValue())).'"'.$attributes.' />';
		$extra = '';
	...
	{
		return "text -> Beispiel: month|label|Bezeichnung|defaultwert|[no_db]|classes|min|max|step";
	}
