...
class rex_xform_value_tel extends rex_xform_value_abstract
{
    ...
    $label = ($this->getElement(2) != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->getElement(2)) . '</label>' : '';	
		$field = '<input'.$classes.' id="'.$this->getFieldId().'" type="tel" name="'.$this->getFieldName().'" value="'.htmlspecialchars(stripslashes($this->getValue())).'" />';
		$extra = '';
	...
	{
		return "text -> Beispiel: tel|label|Bezeichnung|defaultwert|[no_db]|classes";
	}
	...
	function getDefinitions()
	{
		return array(
						...
						'name' => 'tel',
						'values' => array(
									array( 'type' => 'name',   'label' => 'Telefon' ),
									array( 'type' => 'text',    'label' => 'Bezeichnung'),
									array( 'type' => 'text',    'label' => 'Defaultwert'),
									array( 'type' => 'no_db',   'label' => 'Datenbank',  'default' => 1),
									array( 'type' => 'text',    'label' => 'classes'),
								),
						'description' => 'Ein Textfeld zur Eingabe einer Telefonnummer',
						'dbtype' => 'text',
						'famous' => TRUE
						);
