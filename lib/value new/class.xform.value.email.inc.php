...
class rex_xform_value_email extends rex_xform_value_abstract
{
    ...
    $label = ($this->getElement(2) != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->getElement(2)) . '</label>' : '';	
		$field = '<input'.$classes.' id="'.$this->getFieldId().'" type="email" name="'.$this->getFieldName().'" value="'.htmlspecialchars(stripslashes($this->getValue())).'" />';
		$extra = '';
	...
	{
		return "text -> Beispiel: email|label|Bezeichnung|defaultwert|[no_db]|classes";
	}
	...
	function getDefinitions()
	{
		return array(
						...
						'name' => 'email',
						'values' => array(
									array( 'type' => 'name',   'label' => 'E-Mail Feld' ),
