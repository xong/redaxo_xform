...
class rex_xform_value_datetime_local extends rex_xform_value_abstract
{
    ...
    $label = ($this->getElement(2) != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->getElement(2)) . '</label>' : '';	
		$field = '<input'.$classes.' id="'.$this->getFieldId().'" type="datetime-local" name="'.$this->getFieldName().'" value="'.htmlspecialchars(stripslashes($this->getValue())).'"'.$attributes.' />';
		$extra = '';
	...
	{
		return "text -> Beispiel: datetime_local|label|Bezeichnung|defaultwert|[no_db]|classes|min|max|step";
	}
	...
	function getDefinitions()
	{
		return array(
						...
						'name' => 'datetime_local',
						'values' => array(
									array( 'type' => 'name',   'label' => 'Datum und Uhrzeit' ),
