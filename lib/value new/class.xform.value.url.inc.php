...
class rex_xform_value_url extends rex_xform_value_abstract
{
    ...
    $label = ($this->getElement(2) != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->getElement(2)) . '</label>' : '';	
		$field = '<input'.$classes.' id="'.$this->getFieldId().'" type="url" name="'.$this->getFieldName().'" value="'.htmlspecialchars(stripslashes($this->getValue())).'" />';
		$extra = '';
	...
	{
		return "text -> Beispiel: url|label|Bezeichnung|defaultwert|[no_db]|classes";
	}
