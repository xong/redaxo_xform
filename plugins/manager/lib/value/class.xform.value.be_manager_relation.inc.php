...
class rex_xform_value_be_manager_relation extends rex_xform_value_abstract
{
  ...
  function enterObject()
  {
    ...
    if($this->be_em["relation_type"] == 2 || $this->be_em["relation_type"] == 3)
    {
						...
						<a href="#" class="rex-icon-file-bottom" 
				       	...
				       	<span class="rex-widget-column">
							<a href="#" class="rex-icon-file-open" 
								...
								title="'.rex_i18n::translate("xform_relation_choose_entry").'"></a>
							<a href="#" class="rex-icon-file-delete" 
								onclick="xform_manager_deleteDatalist('.$this->getId().',\''.$multiple.'\');return false;" 
								title="'.rex_i18n::translate("xform_relation_delete_entry").'"></a>
				        </span>
				 	    </p>
				    </div>
				  </div>
      ...
      $before = '';
      $after = '';
      $label = ($this->be_em['label'] != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->be_em['label']) . '</label>' : '';	
      $field = $out;
      $extra = '';
    ...
    if($this->be_em["relation_type"] == 4)
    {
      ...
      $after = $text;    
      $label = ($this->be_em['label'] != '') ? '<label'.$classes.' for="' . $this->getFieldId() . '">' . rex_i18n::translate($this->be_em['label']) . '</label>' : '';	
      $field = '<input type="hidden" name="'.$this->getFieldName().'[]" id="REX_RELATION_'.$this->getId().'" />';
      $extra = '';
      $html_id = $this->getHTMLId();
      $name = $this->getName();
